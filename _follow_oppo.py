...
def select_device():
    devices = get_connected_devices()#"734d6104"
    if not devices:
...
def capture_screenshot(device_id):
    filename = f'screenshot_follow_oppo{device_id}.png'
...
def swipe_if_image_detected():
    device_id = "734d6104"  # Predefined device ID
    if not device_id:
    ...
    try:
        while True:
            ...
            if coordinates:
                ...
                time.sleep(0.2)
                simulate_click(device_id, 757, 1580)
                time.sleep(0.2)
            ...
            if coordinates:
                            ...
                            time.sleep(0.5)
                            simulate_click(device_id, 60, 193)
                            time.sleep(0.2)
