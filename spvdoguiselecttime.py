...
def start_swiping():
    ...
    # Get selected delay time
    selected_delay = float(delay_var.get())

    def swipe_loop():
        while not stop_swipe:
            swipe_up(device_id)
            time.sleep(selected_delay)  # Delay between swipes
...
delay_var = tk.StringVar(value="0.2")  # Default delay value
stop_swipe = False
...
delay_label = tk.Label(root, text="Select Delay (seconds):")
delay_label.pack(pady=5)

delay_options = ["0.2", "0.5", "1", "2", "6", "11", "21", "30", "40"]
delay_menu = tk.OptionMenu(root, delay_var, *delay_options)
delay_menu.pack()
